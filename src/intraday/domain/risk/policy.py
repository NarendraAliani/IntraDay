# File: src/intraday/domain/risk/policy.py
#
# Checkpoint 64.24: `RiskEvaluationContext`/`evaluate_order_risk()`
# relocated here verbatim from `trading_engine/risk_engine/evaluator.py`
# (Checkpoint 34 Part 10) so that the ONE canonical risk-gating decision
# function lives in `intraday.domain` — the one layer every part of
# this codebase (trading_engine, application, AND research) is
# permitted to import (`.importlinter` contracts 1-3), instead of being
# re-declared as a "verified port" inside `research/backtesting/`
# (Checkpoint 64.23) purely to satisfy the import-boundary. No logic
# changed; check order, formulas, and docstrings are unchanged from the
# pre-move source — this is a relocation, not a rewrite.
# `trading_engine/risk_engine/evaluator.py` now re-exports this
