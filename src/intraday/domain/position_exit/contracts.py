# File: src/intraday/domain/position_exit/contracts.py
#
# Checkpoint 64.24: `ExitPlan`/`ManagedPosition`/`ExitDecision`/
# `ExitReason`/`PositionLifecycleStatus` relocated here verbatim from
# `trading_engine/position_management/contracts.py` (Checkpoint 42 Part
# 3-4) so that the position-exit lifecycle contracts live in
# `intraday.domain` — the one layer every part of this codebase
# (trading_engine, application, AND research) is permitted to import
# (`.importlinter` contracts 1-3). No logic changed; this is a
# relocation, not a rewrite. `trading_engine/position_management/
# contracts.py` now re-exports these names for backward compatibility.
