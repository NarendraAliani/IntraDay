...
Establish a secure Settings UI and API for operational provider
configuration — Dhan (broker, read-only connectivity only, NO order
...
- `infrastructure/persistence/models.py` — `DhanCredential`,
  ...
  status model); `AuditLogEntry.outcome` widened with `"updated"`.
  Migration `0005_provider_settings.py`.
- `application/repositories/provider_settings.py` — repository
  Protocols; secret-free record types (`has_access_token: bool`, never
  the value).
- `infrastructure/persistence/provider_settings_repositories.py` —
  Django ORM implementations; write-only `None`-means-unchanged
  semantics; audited credential changes in the same transaction as the
  write.
- `application/services/provider_settings.py` — the one real business
...
```
Settings UI (database) value? YES -> source = "DATABASE"
                               NO  -> Environment variable set?
                                       YES -> source = "ENVIRONMENT"
...
`docs/architecture/ARCHITECTURE_DECISIONS.md` decisions 100-105 +
Notes (Checkpoint 22); READMEs updated:
...
## Checkpoint 32 — Runtime Architecture Decision + System-Wide Placeholder Coverage + Reporting Foundation (2026-08-14)

### What was done

- `docs/architecture/RUNTIME_ARCHITECTURE_DECISION.md` (new): a 15-dimension decision matrix comparing three non-Docker options for hosting a persistent WebSocket market-data process (Django Channels/ASGI, a Celery worker, a dedicated standalone Python worker process). Recommendation: a standalone worker process via a Django management command - smallest genuine increment, cleanest fault isolation, reuses this project's own most-proven `manage.py` pattern. Docker's deferral was explicitly re-examined (not merely carried forward) and re-confirmed still valid - nothing about this problem requires containerization. A full persistent-process contract (startup/auth/subscription/heartbeat/reconnect/backoff/shutdown/health/communication boundaries) was defined, not implemented - the next WebSocket-implementation checkpoint builds against it.
- `src/intraday/application/reporting/` (new, application layer, no new bounded context/import-linter contract needed): `ReportMetadata` (one shared contract reused by every report type, reusing `BacktestTrustLevel`/`DataQualityLabel` verbatim rather than inventing parallel enums), `ReportStatus`/`ReportType` enums, `REPORT_CATALOGUE` (all 10 report types from the checkpoint's own catalogue, each with purpose/owner/required-data/UI-surface, honest about which are AVAILABLE/PARTIAL/PLANNED/NOT_YET_IMPLEMENTED - RISK_REPORT and PRODUCTION_REPORT are never marked AVAILABLE, proven by a dedicated test). `backtest_report.py::build_backtest_report_metadata()` maps an existing `BacktestResult` into `ReportMetadata`, copying `trust_level` verbatim (never raising it - proven by a regression test). `market_data_quality_report.py` structures Checkpoint 31's real TRADING_GRADE_BAR findings as queryable data (six `TradingGradeBarCondition` entries with evidence strings) - `current_classification` only becomes `TRADING_GRADE_BAR` when all six are SATISFIED, never inferred from partial progress.
- Frontend: `common/components/CapabilityStatus.tsx` - the ONE reusable placeholder component (title/description/status/blocker/prerequisite/documentationLink/expectedCheckpoint), replacing any need for handcrafted per-page placeholder markup. `features/reports/capabilityRegistry.ts` - the single authoritative list of every major product capability (Research/Market Data/Trading/Notifications groups) with honest states. `features/reports/ReportsOverviewPage.tsx` (new nav entry, "Reports") - report catalogue, Market Data Quality Report (six-condition table), export placeholders (PDF/CSV/JSON, all PLANNED - no document-generation framework introduced), and the full capability registry, all rendered through `CapabilityStatus`.
- Documentation: `docs/architecture/REPORTING_ARCHITECTURE.md` and `docs/architecture/PLACEHOLDER_AND_FEATURE_STATE_ARCHITECTURE.md` (both new), plus short update sections in `ARCHITECTURE.md`, `ARCHITECTURE_DECISIONS.md` (Decisions 138-143), `reports/README.md`, `frontend/reports/README.md`, `control_plane/README.md`, `research/README.md`.
- WebSocket ingestion was explicitly NOT implemented this checkpoint, per its own instruction - only designed and documented.

### Validation

- Backend: ruff/mypy/lint-imports (6/6 contracts kept, no new contract needed)/manage.py check/makemigrations --check/spectacular all clean - no new persisted models, no migration. New tests: 12 in `tests/unit/application/reporting/` covering metadata validation, catalogue completeness (exactly 10 entries, every ReportType covered exactly once), the "never auto-promote trust level" and "never auto-promote SAMPLE_BAR to TRADING_GRADE_BAR" regression assertions Part 17 explicitly required.
- Frontend: tsc/build clean. New tests: 4 for `CapabilityStatus` (label rendering, blocker/prerequisite conditional rendering), 5 for `ReportsOverviewPage` (every catalogue/registry entry renders, classification stays SAMPLE_BAR, no BLOCKED capability shows AVAILABLE, no trading control rendered). Existing `App.test.tsx` unaffected (nav addition is additive).
- Non-redundancy: report-catalogue/status logic exists in exactly one backend place (`application/reporting/contracts.py`); the frontend catalogue is an explicitly-documented presentation mirror, not a competing source of truth; `CapabilityStatus` is the only placeholder-rendering component (grep-verified no other hand-written "coming soon" markup was introduced).
- Trust level: unchanged, still POC. RESEARCH_READY gate: unchanged from Checkpoint 31 (this checkpoint added reporting/placeholder infrastructure, not new validation evidence).
- Trading safety: 0 orders, 0 broker execution calls, 0 position changes, 0 live trading enabled. TRADING_MODE/kill switch/order management untouched. No credentials exposed. No WebSocket ingestion, gap recovery, or live trading implemented - explicitly deferred to a future checkpoint per this checkpoint's own scope boundary.

## Checkpoint 33 — Active Product Readiness Audit + External Research (2026-08-14)

### What was done

A deliberately critical, evidence-based audit answering "what is still missing to make this an actually operable trading platform?" - not another feature-building checkpoint, no implementation code changed.

- Real external research: `dhanhq.co/docs/v2/orders/` fetched live, establishing the real 7-state Dhan order lifecycle (TRANSIT/PENDING/REJECTED/CANCELLED/PART_TRADED/TRADED/EXPIRED - our domain enum has 6, missing TRANSIT and EXPIRED), order placement/modify/cancel/order-book/trade-book endpoints, correlation-ID support, order slicing, and the confirmed hard requirement that order placement/modification/cancellation APIs require static IP whitelisting (previously undocumented anywhere in this project). SEBI's algo-trading circular (Feb 2025, timeline extended Sep 2025) confirmed live against SEBI's own site; substantive content sourced from labeled secondary summaries (primary PDF not machine-extractable this session) - registration thresholds, broker-as-principal liability, algo ID tagging, Indian-server hosting requirement.
- Full implementation map with an 8-state status taxonomy (VERIFIED_IMPLEMENTED/IMPLEMENTED_BUT_NOT_VALIDATED/DESIGNED_NOT_IMPLEMENTED/EXTERNAL_DEPENDENCY/ASSUMPTION/BLOCKED/MISSING/DEFERRED) applied honestly across every subsystem - confirmed `risk_engine`/`order_management`/`execution_management`/`session_management`/`broker_abstraction` remain empty scaffolding (7-8 line `__init__.py` files), no order has ever been placed, no reconciliation exists, no paper trading exists, no kill switch implementation was found anywhere in the repository.
- Full trading-lifecycle audit (19 stages, MARKET OPEN through DAILY REPORT): only 3 stages are more than "designed"; everything from RISK APPROVED onward is completely missing.
- Broker execution gap matrix (18 capabilities), order state machine analysis (missing TRANSIT/EXPIRED states, unaddressed cancellation race since Dhan's DELETE returns async 202), reconciliation design (Dhan as sole authoritative source, not yet implemented), session-model gap (no holiday calendar, confirmed still insufficient for live operation), risk-engine audit (confirmed the single largest gap - RiskLimits is a configuration schema, not an enforcement engine), market-data quality audit beyond TRADING_GRADE_BAR, clock-synchronization audit (no NTP/drift monitoring exists), instrument-master audit (4 hardcoded symbols, no corporate-action handling), strategy research quality audit, backtest-vs-live parity audit (found live and backtest bar aggregation are two independently-written, never-cross-tested code paths), paper-trading requirements design (full-fidelity, not a toy simulation), observability audit (no metrics/tracing/alerting infrastructure), failure/recovery audit (16 named scenarios, systematic gaps), operator-workflow and UX gap analysis, reporting gap analysis (8 of 10 catalogue report types still have no real content).
- Master gap register: 20 gaps classified P0-P3, with explicit IMPLEMENT NOW/RESEARCH FIRST/BLOCKED/WAITING FOR EXTERNAL DEPENDENCY decisions for every P0/P1 item.
- Architecture-assumption review (Part 26): Dhan-only design, standalone worker, Redis role, persistence strategy, report architecture, frontend contracts, strategy-execution ownership, broker abstraction, control-plane authority, market-data reconciliation pattern, paper-trading design, AI agent authority - each explicitly re-examined; no wholesale architecture change recommended, but the Checkpoint 32 persistent-process contract is flagged as needing extension for token-refresh handling.
- **Checkpoint 32's WebSocket-implementation recommendation is explicitly superseded**: the evidence shows paper trading (order state machine + minimal risk engine + reconciliation design) is the higher-value next step, since a trading-grade feed alone would not make the platform meaningfully closer to operable when nothing exists yet to safely act on live data.
- Documentation: `docs/research/ACTIVE_PRODUCT_READINESS_RESEARCH.md` and `docs/architecture/PRODUCT_READINESS_GAP_ANALYSIS.md` (both new), update notes in `ARCHITECTURE.md`, `ARCHITECTURE_DECISIONS.md` (Decisions 144-146).

### Validation

- No implementation code changed this checkpoint - full regression suite re-run to confirm nothing broke: ruff/mypy/lint-imports/manage.py check/makemigrations --check all clean, full pytest suite green, frontend typecheck/build/vitest all green.
...
## Checkpoint 35 — Paper Trading Completion + Full Frontend/CSS Design System Audit (2026-08-15)

### What was implemented

- Backend gap closure from Checkpoint 34's own honest disclosures: automatic ledger persistence (`PaperTradingService` + `DjangoPaperLedgerRepository`, atomic full-state resync after every order, proven with restart-recovery/idempotency tests), authenticated read APIs for orders/trades/positions/funds, a real order-submission endpoint and frontend form (MARKET/LIMIT/SL/SL-M), `BrokerOrderStatusReport.instrument_id` (closing the instrument-level duplicate-order gap, 4 new tests), and a manually-triggerable end-of-session expiry endpoint (`PaperBroker.force_expire_end_of_session()` exposed via API, scheduled auto-trigger still undone - named gap).
- `PaperTradingPage.tsx` rebuilt: real, wired order-entry form and order/trade/position/funds monitor tables, alongside the existing kill-switch controls - every genuinely-available capability now marked AVAILABLE, every remaining gap (scheduled expiry, automatic live-price feed, reconciliation report) marked PARTIAL/NOT_YET_IMPLEMENTED via the shared `CapabilityStatus` component.
- Market-data-to-PaperBroker wiring: deliberately NOT automated this checkpoint (`PAPER_TRADING_ARCHITECTURE.md`'s Checkpoint 35 update explains why - avoiding an implicit coupling to SAMPLE_BAR-quality data without a reviewed design, per Part 8's own explicit "do not promote SAMPLE_BAR" warning).
- System-wide frontend/CSS audit (`docs/architecture/FRONTEND_DESIGN_SYSTEM.md`): headline finding - zero `@media` rules existed anywhere in the frontend before this checkpoint. Added a full design-token set (spacing/typography/radius/shadow/focus/motion/z-index scales, paper-vs-live color tokens), global responsive base (768px/480px/640px breakpoints), and a global `:focus-visible` rule (no visible keyboard focus state existed anywhere before this checkpoint). Found and fixed 9 hardcoded color literals, including a stale hex fallback that didn't even match its own token's real value - a genuine bug, not just a style nit.
- CSS quality gate (`frontend/src/app/styles.quality.test.ts`, 8 tests): no duplicate token definitions, no hardcoded colors outside `:root`, no duplicate rule blocks, at least one responsive rule exists, a visible focus state exists, no inline `style=` usage anywhere.
- External research: Dhan's official Sandbox API (a genuinely new finding - fully simulated environment, no static IP required, fixed-price fills, reportedly no live streaming) surfaced via WebSearch, `VERIFIED_SECONDARY` only (a direct fetch of the primary sandbox doc page returned no content this session) - flagged as a promising future path for adapter validation before any live-execution work, explicitly not yet acted on. Professional trading-dashboard UX patterns researched and selectively applied (status-by-form, KPI-first layout, danger visibility) - behavioral/engagement patterns explicitly NOT applied as premature for a paper/proving-ground surface.

### Validation

- Backend: ruff/mypy/lint-imports (6/6 contracts kept - a real architecture violation was caught and fixed: an early draft of `paper_trading_runtime.py` lived in `application/services/` and imported `infrastructure` directly, violating contract 6; moved to `infrastructure/api/`, matching `settings_views.py`'s own established "infrastructure/api composes application + infrastructure" precedent)/manage.py check/makemigrations --check/spectacular all clean. New tests: ~35 new backend tests (ledger persistence, paper-trading API vertical slices including kill-switch interaction and session expiry, instrument-level duplicate-order protection).
- Frontend: tsc/build clean, 104 vitest tests passing (11 new: 9 PaperTradingPage rewrite covering real order entry/monitor tables, 8 CSS quality gate - some overlap in counting, net +10 test files worth of new coverage). OpenAPI schema + frontend types regenerated for the new paper-trading and expiry endpoints.
- Browser automation: re-confirmed unavailable (same as every prior checkpoint) - this checkpoint's CSS/UX audit is explicitly a structural/code-level audit, not a rendered-pixel review, stated plainly rather than implied otherwise.
- Trading safety: 0 real orders, 0 broker execution calls, 0 positions changed at a real broker, 0 LIVE mode enablement. TRADING_MODE unchanged, kill switch unchanged in behavior (still real, still tested). No credentials exposed.
- Honest, named remaining gaps: no scheduler triggers end-of-session expiry automatically; no automatic live-price feed into the paper broker; no reconciliation-report UI; per-page mobile-width verification below 480px, measured color-contrast ratios, and full keyboard tab-order walkthrough were not performed (browser automation unavailable) - all disclosed in `FRONTEND_DESIGN_SYSTEM.md`/`PAPER_TRADING_ARCHITECTURE.md`, not hidden.
